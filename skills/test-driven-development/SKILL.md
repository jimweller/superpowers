...
- RED: Write failing test. Verify it fails for the expected reason. If the failure is wrong (import error, typo, wrong assertion), fix the test and re-verify before proceeding.
- GREEN: Write minimal code to pass. Run ALL tests, not just the new one. If anything breaks, fix before moving on.
- REFACTOR: Clean up. Re-run tests. If anything breaks, revert the refactor and try a different approach.
